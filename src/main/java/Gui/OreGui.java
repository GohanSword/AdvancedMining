...
public class OreGui implements CommandExecutor {
    ...
    public boolean  onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if(player.hasPermission("awc.use")) {
            Gui menu = new Gui(6, "§f§lORE §b§lMenu");
            GuiItem boutiqueventedebois = ItemBuilder.from(Material.WOODEN_AXE).setName("§3§lBoutique de vente de Minerais").setLore("§7Cliquez pour ouvrir le magasin", "test").asGuiItem(event -> {
